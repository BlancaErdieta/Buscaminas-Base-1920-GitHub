...
public class ControlJuego {
	...
	private int calculoMinasAdjuntas(int i, int j) {
		...
		for (int hor = Math.max(0, i-1); hor <=Math.min(i+1, LADO_TABLERO-1); hor++) {
			for (int ver = Math.max(0, j-1); ver <= Math.min(j+1, LADO_TABLERO-1); ver++) {
				if (tablero[hor][ver]==MINA) {
					numeroMinas++;
				}
			}
		}
	...
	public boolean esFinJuego() {
		int celdasSinMina = (LADO_TABLERO * LADO_TABLERO) - 20;
		if (puntuacion == celdasSinMina) {
			return true;
		} else {
			return false;
		}
